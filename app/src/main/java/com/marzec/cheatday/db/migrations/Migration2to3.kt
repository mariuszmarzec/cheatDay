...
class Migration2to3 : Migration(2, 3) {
    override fun migrate(database: SupportSQLiteDatabase) {
        ...
        database.execSQL("INSERT INTO `temp` (`type`, `count`, `max`, `email`) " +
                "SELECT days.type, " +
                "days.count, " +
                "days.max, " +
                "users.email FROM days INNER JOIN users ON days.user_id=users.uuid")
        ...
        database.execSQL("CREATE TABLE IF NOT EXISTS `${DayEntity.NAME}` (`" +
        ...
        database.execSQL("CREATE TABLE IF NOT EXISTS `${WeightResultEntity.NAME}` (" +
                "`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, " +
                "`value` REAL NOT NULL, " +
        ...
        database.execSQL("INSERT INTO `days` (`type`, `count`, `max`, `user_id`) " +
                "SELECT `temp`.type, `temp`.count, `temp`.max, users.id FROM `temp` INNER JOIN users ON `temp`.email=users.email")
